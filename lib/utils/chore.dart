...
void openProjectLink() async {
  Uri uri = Uri.parse("https://github.com/obineller411/apprhymeX");
  if (await canLaunchUrl(uri)) {
    await launchUrl(uri);
  } else {
    await Clipboard.setData(
        const ClipboardData(text: "https://github.com/obineller411/apprhymeX"));
...
Future<String?> pickDirectory() async {
  ...
  if (await Permission.manageExternalStorage.request().isGranted) {
    ...
    // 权限被拒绝
    LogToast.error("未授予存储权限", "请在设置中授予AppRhymeX存储权限",
