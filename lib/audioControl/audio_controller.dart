...
class AudioHandler extends GetxController {
  ...
  Future<void> _init() async {
    ...
    player.playbackEventStream.listen((event) {},
      ...
      if (playingMusic.value != null) {
        ...
        if (AudioPlaybackRecoveryHelper().shouldSkipMusic(musicId)) {
          ...
          Future.delayed(AudioPlaybackRecoveryHelper().getErrorRecoveryDelay(musicId), () {
            if (isPlaying) {
              seekToNext();
            }
          });
    ...
    player.currentIndexStream.listen((index) async {
      ...
      if (musicList[index].shouldUpdate()) {
        final musicId = musicList[index].extra ?? musicList[index].info.name;
        ...
        if (AudioPlaybackRecoveryHelper().shouldSkipMusic(musicId)) {
          ...
          // 获取智能延迟后跳过
          final delay = AudioPlaybackRecoveryHelper().getErrorRecoveryDelay(musicId);
  ...
  Future<void> replacePlayingMusic(Quality quality_) async {
    try {
      ...
      if (index != -1) {
        // 添加调试信息
        globalTalker.info("[Music Handler] 手动切换音质: ${quality_.short} (level: ${quality_.level}) for ${playingMusic.value!.info.name}");
        ...
        // 添加额外的调试信息
        globalTalker.info("[Music Handler] 音质切换完成，当前音质: ${playingMusic.value?.currentQuality.value?.short}");
      }
    } catch (e) {
      globalTalker
  ...
  Future<void> clearReplaceMusicAll(List<MusicContainer> musics) async {
    ...
    await _clearReplaceMusicAllock.synchronized(() async {
      ...
      // 将所有音乐添加进去，但是先不获取链接，使用lazy load
      musicList.addAll(musics);
      await audioSourceListLock.synchronized(() async {
        await audioSourceList
            .addAll(musics.map((e) => e.audioSource).toList());
      });
  ...
  void updatePlayingMusic({MusicContainer? music}) {
    ...
    if (music != null) {
      ...
      try {
        newMusic = musicList[player.currentIndex!];
      } catch (e) {
        globalTalker
    ...
    if (globalCustomAudioHandler != null && playingMusic.value != null) {
      Uri? artUri;
      if (playingMusic.value!.info.artPic != null) {
...
class AudioUiController extends GetxController {
  ...
  RxBool isQualityMenuOpen = false.obs;
  AudioUiController() {
    // 初始化late
