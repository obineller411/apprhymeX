    ...
    match resp {
        Ok(resp) => {
            ...
            if !resp.status().is_success() {
                return Err(anyhow::anyhow!("Primary URL returned non-success status: {}", resp.status()));
            }
            
            // 检查响应内容是否为空
            let text = resp.text().await?;
            if text.trim().is_empty() {
                return Err(anyhow::anyhow!("Primary URL returned empty response"));
            }
            ...
            match serde_json::from_str::<Release>(&text) {
                ...
                Err(e) => {
                    // 尝试备用URL
                    get_release_from_backup(back_url).await
                }
            }
        }
        Err(_) => {
...
async fn get_release_from_backup(back_url: &str) -> Result<Release, anyhow::Error> {
    ...
    if !back_resp.status().is_success() {
        return Err(anyhow::anyhow!("Backup URL returned non-success status: {}", back_resp.status()));
    }

    let text = back_resp.text().await?;
    if text.trim().is_empty() {
        return Err(anyhow::anyhow!("Backup URL returned empty response"));
    }

    match serde_json::from_str::<Release>(&text) {
        Ok(release) => Ok(release),
        Err(e) => Err(anyhow::anyhow!("Failed to parse JSON from backup URL: {}, response: {}", e, text))
    }
}
